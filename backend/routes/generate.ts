...
import { callAI, MAX_RETRIES } from '../lib/bedrock.js';
import { loadPrompt, fillTemplate } from '../lib/prompts.js';
import { fetchFullMetadata, formatCastList } from '../lib/tubiSearch.js';
import { parseArticleJSON, runArticleQualityGate, serializeArticleOutput } from '../lib/quality-gate.js';
import { runBrandCheck } from '../lib/bedrock.js';
...
router.post('/generate', async (req: Request, res: Response) => {
  const { mode, contentId } = req.body as { mode: string; contentId: string };

  if (mode !== 'single') {
    res.status(400).json({ error: 'Only mode=single is supported in this version' });
    return;
  }
  if (!contentId) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES + 1; attempt++) {
      ...
      try {
        ...
        if (attempt > MAX_RETRIES) {
          sse(res, { type: 'error', message: 'AI generation failed after all retries' });
          res.end(); return;
        }
        continue;
      }
      ...
      try {
        ...
        // brand check failure is non-blocking — proceed
      }
